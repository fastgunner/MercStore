...
And last, but definitely not least, the A3 community!
...
A3MRaid1 SetSimpleTaskDescription ["The Takistani Government requests assistance in dealing with a foreign terrorist cell that has taken over a compound. The Takistani Government cannot risk an international incident by stepping in, but the Takistani police forces are unprepared and ill equipped to handle this situation. OPSG is hereby contracted to eliminate all terror cells operating in Takistan. ", "Eliminate Terror Cell", "Terrorist Compound"];
...
A3MRaid2 SetSimpleTaskDescription ["The Takistani Government requests assistance in dealing with a foreign terrorist cell that has taken over an entire city. The Takistani Government cannot risk an international incident by stepping in, but the Takistani police forces are unprepared and ill equipped to handle this situation. OPSG is hereby contracted to eliminate all terror cells operating in Takistan. ", "Eliminate Terror Cell", "Terrorist City"];
...
playSound "A3M_Deuce";   
titleText ["Travelling to the C-12 Air Facility | Orion Private Security Group / Takistan Army", "BLACK FADED",10];
titleFadeOut 5;
...
[
		...
		[
			["Takistan ","align = 'center' shadow = '1' size = '0.7' font='PuristaBold'"],
...
with uiNamespace do {
    ...
    _object setObjectTexture [0,"\A3M_PMCSimulator_Taki\video\NFLVid1.ogv"]; 
    1100 cutRsc ["RscMissionScreen","PLAIN"];
    _scr = BIS_RscMissionScreen displayCtrl 1100;
    _scr ctrlSetPosition [-10,-10,0,0];
    _scr ctrlSetText "\A3M_PMCSimulator_Taki\video\NFLVid1.ogv";
...
if (isNil "TargetPlayer") then { hint " Please Select an Active Operative First"} else {
	if (isNil "GlobalPushAmount") then { Hint "You must first set an amount."} else {
		 if (GlobalPushAmount <= 0 ) then { Hint "You Can't Do That."} else {
			if (GlobalPushAmount < 1) then {Hint "You must deduct more than that, pussyfoot!"} else {
				if (GlobalPushAmount >= 1) then {
...
RBduty SetSimpleTaskDescription ["Stand at the checkpoint and perform stop and search checkpoint duties with the authority of the Takistani Army.", "Move To Checkpoint Duty", " Checkpoint C-12N"];
...
RBduty2 SetSimpleTaskDescription ["Wait for cars to stop at the checkpoint and perform stop and search checkpoint duties with the authority of the Taki Government.", "Perform Checkpoint Duty", " Checkpoint C-12N"];
RBduty2 SetSimpleTaskDestination (getMarkerPos "RBC");
RBduty2 setTaskState "Assigned"; 

titleText ["Welcome to the C-12 South Checkpoint. Perform stop and search duties on Taki Nationals. Be suspicious of terroristic threats.", "PLAIN DOWN"];
...
greet= RBVeh addAction ["Greeting", {Hint "Hello. How are you today sir? Im a Taki Citizen." }];
...
RBveh addAction ["Search", {RBVehD LeaveVehicle RBVeh; hint "I did nothing to you! Why are you judging me like this? Don't judge me! "; sleep 2; hint "Searching Car....."; sleep 5; hint "Found open bottle of Russian vodka"; RBVehD addAction ["Return to Vehicle", {RBVehD moveInDriver RBVeh} ]; RBVehD addAction ["Arrest for DWI", { mantype= 4; publicVariable "mantype"; Hint "Abreast? Arrest meee? Fuck YOU! You aren't GAWD!"; sleep 2; hint "Escort the Suspect to Taki jail. Alternately, use the Fast Transport option in the action menu to fast travel the suspect to Taki Jail."; sleep 2; [] call A3M_fnc_jailtrigger;}];}];
...
mantype= "M1";
removeAllActions RBVeh; 
RBveh addAction ["Greeting", {Hint "Hello sir. All is okay here, yes? I am Taki Citizen." }];
...
RBveh addAction ["Search", {RBVehD LeaveVehicle RBVeh; hint "I better call my lawyer. I'm not saying anything until my attorney arrives. Oh my God."; sleep 2.0; hint "Searching Car....."; sleep 6; hint "You found a load of cocaine"; sleep 2; hint "Driver: Step out of the vehicle! \n \n If the driver refuses to exit, shoot the vehicle's tires and disable it!";
		...
		RBVehD addAction ["Sir, You are under arrest...", { mantype= "M4"; publicVariable "mantype"; Hint "Oh God...Oh My God....Ohhhh Shit. Fuck...FUCK ME! "; sleep 3.0; hint "Transport the Criminal to Taki Jail. Alternately, use the Fast Transport option in the action menu to fast travel the suspect to Taki Jail."; [] call A3M_fnc_jailtrigger; }];}];	
...
RBveh addAction ["Search", { RBVehD LeaveVehicle RBVeh; hint "Dis No Gut."; sleep 2.0; hint "Searching Car....."; sleep 5; hint "Found Multiple Identifications. Upon further review, the Taki Citizen Number on the Taki ID doesn't check out..."; RBVehD addAction ["Return to Vehicle", {RBVehD moveInDriver RBVeh}]; RBVehD addAction ["Arrest for Illegal Immigration", { mantype= "M4"; publicVariable "mantype"; Hint "Fock you mane. You cops are always trying to keepa de mano down!  "; sleep 3.0; hint "Escort the Illegal Alien to Taki Jail. Alternately, use the Fast Transport option in the action menu to fast travel the suspect to Taki Jail."; [] call A3M_fnc_jailtrigger; } ]; } ];
...
removeAllActions RBVeh; 
RBVeh setDamage 0.3; 
RBveh addAction ["Greeting", {Hint "Im a Taki Citizen. What the fuck do you want? Fuck outta my way, bitch!" }];
backscatter= RBVeh addAction ["Backscatter Scanner", {Hint "Anomaly Detected"} ]; 
...
mantype= "M2"; 
removeAllActions RBVeh; 
RBveh addAction ["Greeting", {Hint "I'm a Taki Citizen. Let me pass." }];
backscatter= RBVeh addAction ["Backscatter Scanner", {Hint "Anomaly Detected"}]; 
...
RBveh addAction ["Greeting", {Hint "I'm a Taki Citizen. Let me pass. Immediately" }];
...
RBveh addAction ["Greeting", {Hint "Hello! I'm a Taki Citizen." }];
...
mantype= "M1"; 
removeAllActions RBVeh; 
greet= RBVeh addAction ["Greeting", {Hint "How are you today sir. Im a Taki Citizen." }];
backscatter= RBVeh addAction ["Backscatter Scanner", {Hint "Anomaly Detected"} ]; 
inspect= RBVeh addAction ["Inspect Vehicle", {Hint "Visual Inspection: \n The vehicle appears clean and in good repair. \n Smells:\n The vehicle doesn't smell of anything at all.  \n Driver Attitude: \n Driver appears calm and cooperative"} ];
search= RBVeh addAction ["Search", {RBVehD LeaveVehicle RBVeh; hint "Officer, is this really needed? This is just stupid. I know my rights! Call my lawyer! "; hint "Searching Car....."; sleep 6; hint "You found a load of heroin."; sleep 2; hint "Driver: Step out of the vehicle! \n \n If the driver refuses to exit, shoot the vehicle's tires and disable it!"; RBVehD addAction ["Return to Vehicle", {RBVehD moveInDriver RBVeh} ]; RBVehD addAction ["Sir, You are under arrest...", { mantype= "M4"; publicVariable "mantype"; Hint "Oh Mother of God. I've really done it now."; sleep 3.0; hint "Transport the Criminal to Taki Jail.  Alternately, use the Fast Transport option in the action menu to fast travel the suspect to Taki Jail.";[] call A3M_fnc_jailtrigger;}];}]; 
goSouth= RBVeh addAction ["Allow to Proceed", {hint "Thank you sir, I'll be moving on now"; [] Call DoExitStopSouth; }];  
...
A3M_Booked_MP = {
hint "Prisoner has been booked into Taki Police Custody. Thank you!";
...
ALTEscort=player createSimpleTask ["Escort Prisoner to Taki Jail"]; 
ALTEscort SetSimpleTaskDescription ["Escort the Taki Law Breaker to Taki Jail.", "Jail Transport", "Taki Jail"];
